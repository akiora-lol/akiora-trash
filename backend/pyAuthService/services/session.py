...
class SessionService:
    ...
    async def create_session(
        ...
        await self.redis.setex(
            f"{self.prefix}{session_data.id}:user_id:{session_data.user_id}",
            int(
                (
    ...
    async def get_session(self, session_id: UUID) -> Optional[SessionData]:
        ...
        await self.redis.setex(
            f"{self.prefix}{session_id}:user_id:{data.user_id}",
            int((data.astimezone(tz=UTC) - datetime.now(tz=UTC)).total_seconds()),
            data.model_dump_json(),
        )

        return data

    async def update_session(self, session_id: UUID, **kwargs) -> Optional[SessionData]:
        ...
        await session.save()
        await self.redis.setex(
            f"{self.prefix}{session_id}:user_id:{session.user_id}",
            int(
                (
